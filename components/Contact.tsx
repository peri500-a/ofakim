          ...
          <div className="lg:col-span-2 space-y-8 order-2 lg:order-1">
            <div className="bg-gray-900/50 p-8 rounded-3xl border border-white/5 space-y-8">
              ...
              <div className="space-y-6">
                <a href={`tel:${PHONE_NUMBER}`} className="flex items-center gap-4 group focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-xl p-2">
                ...
                <a href={`mailto:${PROFESSIONAL_EMAIL}`} className="flex items-center gap-4 group focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-xl p-2">
                  <div className="w-12 h-12 bg-blue-600/10 text-blue-500 rounded-xl flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-all duration-300">
              ...
              <div className="pt-6 border-t border-white/5">
                <a 
                  ...
                  className="flex items-center justify-center gap-3 w-full bg-green-600 hover:bg-green-500 text-white font-bold py-4 rounded-2xl transition-all shadow-lg shadow-green-900/20 transform hover:-translate-y-1 focus:outline-none focus:ring-4 focus:ring-green-400"
                >
              ...
              {status === 'success' ? (
                <div className="text-center py-12 animate-fade-in" role="alert">
                  ...
                  <button 
                    ...
                    className="text-blue-400 font-bold hover:underline focus:outline-none focus:ring-2 focus:ring-blue-500 rounded p-1"
                  >
                    שליחת הודעה נוספת
                  </button>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-6" aria-label="טופס יצירת קשר">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label htmlFor="name" className="text-sm font-bold text-gray-400 mr-2 block">שם מלא</label>
                      <input 
                        id="name"
                    ...
                    <div className="space-y-2">
                      <label htmlFor="phone" className="text-sm font-bold text-gray-400 mr-2 block">טלפון ליצירת קשר</label>
                      <input 
                        id="phone"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label htmlFor="service" className="text-sm font-bold text-gray-400 mr-2 block">סוג השירות המבוקש</label>
                      <div className="relative">
                        <select 
                          id="service"
                          name="service" value={formData.service} onChange={handleChange} required
                          className="w-full bg-gray-950 border border-white/5 rounded-2xl px-6 py-4 text-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500 transition-all outline-none appearance-none"
                          aria-required="true"
                        >
                  ...
                  <div className="space-y-2">
                    <label htmlFor="message" className="text-sm font-bold text-gray-400 mr-2 block">ספרו לנו קצת על הנכס</label>
                    <textarea 
                      id="message"
                      name="message" value={formData.message} onChange={handleChange} rows={4}
                      className="w-full bg-gray-950 border border-white/5 rounded-2xl px-6 py-4 text-white focus:border-blue-500 focus:ring-2 focus:ring-blue-500 transition-all outline-none resize-none"
                  ...
                  <button 
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black text-xl py-5 rounded-2xl transition-all shadow-xl shadow-blue-900/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 focus:outline-none focus:ring-4 focus:ring-blue-400"
                  >
                    {status === 'submitting' ? (
                      <>
                        <svg className="animate-spin h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" aria-hidden="true"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        שולח פנייה...
                      </>
                    ) : 'שלחו הצעה עבורי'}
                  </button>
                  ...
                  {status === 'error' && (
                    <p className="text-red-400 text-center text-sm font-bold" role="alert">אופס! קרתה תקלה. נסו שוב או פנו אלינו בוואטסאפ.</p>
                  )}
                </form>
              )}
            </div>
